...
<div class="painel" style="max-width:600px;">
    <form method="post" action="<?= url('/producao') ?>">
            ...
            <div class="campo">
                ...
                <select id="etapa" name="etapa">
                    <?php foreach ($etapas as $slug => $nome): ?>
                        <option value="<?= e($slug) ?>" <?= $slug === 'projeto' ? 'selected' : '' ?>><?= e($nome) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
        ...
        <div style="display:flex;gap:12px;margin-top:20px;">
            <button type="submit" class="botao" style="width:auto;padding:12px 28px;">Criar projeto</button>
            <a href="<?= url('/producao') ?>" class="botao botao-secundario" style="width:auto;padding:12px 28px;text-decoration:none;text-align:center;">Cancelar</a>
        </div>
    </form>
</div>
