...
<div class="painel">
    <div style="display:flex;justify-content:space-between;align-items:center;gap:16px;margin-bottom:20px;flex-wrap:wrap;">
        <form method="get" action="<?= url('/financeiro/pagar') ?>" style="display:flex;gap:8px;">
        ...
        <?php if (podeAcessar('financeiro.editar')): ?>
            <a href="<?= url('/financeiro/pagar/novo') ?>" class="botao" style="width:auto;text-decoration:none;padding:10px 20px;">+ Nova conta a pagar</a>
        <?php endif; ?>
    </div>
    ...
    <?php else: ?>
        <table class="tabela-elegante">
            <thead><tr><th>Fornecedor</th><th>Categoria</th><th>Descrição</th><th>Valor</th><th>Vencimento</th><th>Status</th><th></th></tr></thead>
            <tbody>
                <?php foreach ($contas as $c): ?>
                    <tr>
                        <td><?= $c['fornecedor_id'] ? '<a href="' . url('/fornecedores/' . $c['fornecedor_id']) . '">' . e($c['fornecedor_nome']) . '</a>' : '-' ?></td>
                        <td><?= e($c['categoria'] ?: '-') ?></td>
                        <td><?= e($c['descricao']) ?></td>
                        ...
                        <td>
                            <?php if ($c['status'] === 'pendente' && podeAcessar('financeiro.editar')): ?>
                                <form method="post" action="<?= url('/financeiro/pagar/' . $c['id'] . '/pago') ?>" onsubmit="return confirm('Confirmar pagamento?');">
                                    <?= Csrf::field() ?>
